...
from deepkt.model import SaintModel
...
def run(args):
    ...
    for epoch in range(args.epoch):
        deepkt.utils.train_epoch(saint, train_dataloader, optimizer, loss_func,
                                 device)
        deepkt.utils.eval_epoch(saint, test_dataloader, loss_func, deepkt.utils.sakt_eval, device)
        scheduler.step()
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--num_heads",
                            ...
                            type=int,
                            required=False)
    arg_parser.add_argument("--num_enc",
                            dest="num_enc",
                            default=4,
                            type=int,
                            required=False)
