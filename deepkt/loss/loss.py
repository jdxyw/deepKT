...
class DKTPlusLoss(nn.Module):
    def __init__(self, gamma=0.1, reg1=0.03, reg2=1.0):
    ...
    def forward(self, logits, targets, qid, mask, device="cpu"):
        ...
        preds = torch.squeeze(preds)
        ones = torch.ones(targets.size(), device=device)
        loss1 = -torch.sum(mask * targets * torch.log(preds) + mask *
                           (ones - targets) * torch.log(ones - preds))

        loss2 = -torch.sum(mask[:, :-1] * targets[:, :-1] *
                           torch.log(preds[:, 1:]) + mask[:, :-1] *
                           (ones[:, 1:] - targets[:, :-1]) *
